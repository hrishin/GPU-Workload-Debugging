...
"""
    ...
    def apply_gpu_operator_fix(self, release_name: str = "gpu-operator-1758912452", dry_run: bool = False) -> bool:
        """Apply fix by upgrading GPU operator Helm chart with corrected values"""
        print("🔧 Applying GPU operator fix...")
        
        # Check if Helm is available
        try:
            helm_check = subprocess.run(['helm', 'version'], capture_output=True, text=True)
            if helm_check.returncode != 0:
                print("❌ Helm is not available or not configured")
                return False
        except FileNotFoundError:
            print("❌ Helm command not found")
            return False
        
        # Check if fixed_values.yaml exists
        # Try multiple possible locations for the fixed_values.yaml file
        possible_paths = [
            Path(__file__).parent.parent / "fixed_values.yaml",  # If script is in scripts/ subdirectory
            Path("fixed_values.yaml"),  # Current working directory
            Path(__file__).parent / "fixed_values.yaml",  # Same directory as script
        ]
        
        fixed_values_path = None
        for path in possible_paths:
            if path.exists():
                fixed_values_path = path
                break
        
        if not fixed_values_path:
            print(f"❌ Fixed values file not found. Tried locations:")
            for path in possible_paths:
                print(f"   - {path.absolute()}")
            return False
        
        print(f"✅ Found fixed values file at: {fixed_values_path.absolute()}")
        
        # Use the provided release name
        print(f"📦 Using GPU operator release: {release_name}")
        
        # Get current values from the chart
        try:
            print("📥 Getting current chart values...")
            get_values_cmd = [
                'helm', 'get', 'values', release_name, '-n', 'gpu-operator', 
                '--all', '-o', 'yaml'
            ]
            
            print(f"Running: {' '.join(get_values_cmd)}")
            get_values_result = subprocess.run(get_values_cmd, capture_output=True, text=True, check=True)
            
            if get_values_result.returncode != 0:
                print(f"❌ Failed to get current values: {get_values_result.stderr}")
                return False
            
            current_values = get_values_result.stdout
            print("✅ Successfully retrieved current values")
            
        except subprocess.CalledProcessError as e:
            print(f"❌ Failed to get current values: {e.stderr}")
            return False
        except Exception as e:
            print(f"❌ Unexpected error getting current values: {e}")
            return False
        
        # Load and merge values
        try:
            print("🔄 Merging current values with fixed values...")
            
            # Load current values
            current_yaml = yaml.safe_load(current_values) if current_values.strip() else {}
            
            # Load fixed values
            with open(fixed_values_path, 'r') as f:
                fixed_yaml = yaml.safe_load(f)
            
            # Clean up any None values from fixed_yaml
            if fixed_yaml:
                fixed_yaml = {k: v for k, v in fixed_yaml.items() if v is not None}
            
            print(f"🔍 Debug - Current values keys: {list(current_yaml.keys()) if current_yaml else 'None'}")
            print(f"🔍 Debug - Fixed values keys: {list(fixed_yaml.keys()) if fixed_yaml else 'None'}")
            if 'validator' in current_yaml:
                print(f"🔍 Debug - Current validator: {current_yaml['validator']}")
            if 'validator' in fixed_yaml:
                print(f"🔍 Debug - Fixed validator: {fixed_yaml['validator']}")
            
            # Deep merge the values (fixed values override current values)
            merged_values = self._deep_merge(current_yaml, fixed_yaml)
            
            print(f"🔍 Debug - Merged values keys: {list(merged_values.keys()) if merged_values else 'None'}")
            if 'validator' in merged_values:
                print(f"🔍 Debug - Merged validator: {merged_values['validator']}")
            
            # Create temporary merged values file in the same directory as fixed_values.yaml
            merged_values_path = fixed_values_path.parent / "merged_values.yaml"
            with open(merged_values_path, 'w') as f:
                yaml.dump(merged_values, f, default_flow_style=False)
            
            print("✅ Successfully merged values")
            print(f"📄 Merged values saved to: {merged_values_path}")
            
        except Exception as e:
            print(f"❌ Failed to merge values: {e}")
            return False
        
        # Apply the helm upgrade with merged values
        try:
            if dry_run:
                print(f"🚀 [DRY RUN] Would upgrade GPU operator with merged values...")
            else:
                print(f"🚀 Upgrading GPU operator with merged values...")
            
            upgrade_cmd = [
                'helm', 'upgrade', release_name, 'nvidia/gpu-operator',
                '-n', 'gpu-operator', '-f', str(merged_values_path)
            ]
            
            if dry_run:
                upgrade_cmd.append('--dry-run')
            
            print(f"Running: {' '.join(upgrade_cmd)}")
            result = subprocess.run(upgrade_cmd, capture_output=True, text=True, check=True)
            
            print("✅ GPU operator upgrade completed successfully")
            ...
            # Clean up temporary merged values file
            try:
                merged_values_path.unlink()
                print("🧹 Cleaned up temporary merged values file")
            except:
                pass
    ...
    def _deep_merge(self, base_dict: dict, override_dict: dict) -> dict:
        ...
        for key, value in override_dict.items():
            # Skip None/null values in override_dict to avoid overriding existing values
            if value is None:
                continue
...
def main():
    ...
    parser.add_argument('--fix', action='store_true',
                       help='Apply fix by upgrading GPU operator Helm chart with corrected values')
    parser.add_argument('--release-name', default='gpu-operator-1758912452',
                       help='GPU operator Helm release name (default: gpu-operator-1758912452)')
    ...
    try:
        ...
        # If --fix flag is provided, apply the fix and exit
        if args.fix:
            success = analyzer.apply_gpu_operator_fix(args.release_name, args.dry_run)
